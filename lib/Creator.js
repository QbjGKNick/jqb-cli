...
class Creator {
  ...
  async download(repo, tag) {
    ...
    // 2. 把资源下载到某个路径上（后续可以增加缓存功能，应该下载到系统目录中，稍后可以再使用ejs handlerbar 去渲染模板 最后生成结果 再写入）

    // 放到系统文件中 -> 模板 和用户的其他选择 => 生成结果 放到当前目录下
    await wrapLoading(this.downloadGitRepo(requestUrl, path.resolve(process.cwd(), `${repo}@${tag}`)), 'waiting download repo...')
    return this.target
  }
